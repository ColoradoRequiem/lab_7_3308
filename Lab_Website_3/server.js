...
app.get('/player_info/select_player', function (req, res) {
	player_choice = req.query.player_choice
	var selectPlayer = 'SELECT * FROM football_players WHERE id = $1;';
	var allPlayers = 'SELECT * FROM football_players;';
	var countGames = 'SELECT COUNT(*) FROM football_games WHERE $1 = ANY(players);';
	db.task('get-everything', task => {
		return task.batch([
			task.any(selectPlayer, player_choice),
			task.any(allPlayers),
			task.any(countGames, player_choice)])
	})
	.then(info => {
		res.render('pages/player_info',
		{
			my_title: "Player Info",
			selected_player: info[0][0],
			data: info[1],
			gameCount: info[2][0]
		})
	})
